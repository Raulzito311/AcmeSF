...
class EmprestimoDTO {
	...
	public function validarDados(): void {
		...
		if ($this->valorEmprestimo < 500 || $this->valorEmprestimo > 50000)
			throw new DataException("O valor do empréstimo deve ser entre R$ 500,00 e R$ 50.000,00");
	}
	...
	public static function of(object $body): EmprestimoDTO {
		...
		if (isset($body->valorEmprestimo)) {
			try {
				...
				throw new DataException("O valor do emprestimo deve ser um número decimal (valorEmprestimo)", $ex);
			}
		}
		if (isset($body->dataHora)) {
			try {
				$dto->dataHora = $body->dataHora;
				if (!DateUtil::validateDate($dto->dataHora)) // TODO: Fazer isso funcionar
					throw new Exception();
			} catch (\Throwable $ex) {
				throw new DataException("A data e hora deve ser uma string de data válida (dataHora)", $ex);
			}
		}
